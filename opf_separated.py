...
def opf_mv(dict_input_mv, objective = "obj_prop"):
    '''
    optimize MV nodes using aggregated load and sgen profiles
    '''
    #------------------------  Load Profiles ----------------------------------
    # sgen capacity
    ...
    # bus 4, 8 & 12
    m.addConstr(P_trafo_mv[0] == -p_mv[0])
    m.addConstr(Q_trafo_mv[0] == -q_mv[0])
    m.addConstr(P_trafo_mv[1] == -p_mv[4])
    m.addConstr(Q_trafo_mv[1] == -q_mv[4])
    m.addConstr(P_trafo_mv[2] == -p_mv[8])
    m.addConstr(Q_trafo_mv[2] == -q_mv[8])
    # -------------- objective function --------------------------------------
    ...
    dict_output["v2_trafo"] = np.array([v2_mv[1].X, v2_mv[5].X, v2_mv[9].X])
    dict_output["p_sgen_mv"] = np.array([p_sgen_mv[i].X for i in range(n_bus_mv)]) * np.array([0,1,1,1]*3)
    dict_output["q_sgen_mv"] = np.array([q_sgen_mv[i].X for i in range(n_bus_mv)]) * np.array([0,1,1,1]*3)
...
def opf_lv(dict_input_lv, v2_trafo, P_trafo_mv, Q_trafo_mv, objective = "obj_prop"):
    '''
    They can be solved together because of no mutual impact
    '''
    #------------------------  Load Profiles ----------------------------------
    # sgen capacity
    cap_sgen_lv1 = dict_input_lv["cap_sgen_lv1"]
    cap_sgen_lv2 = dict_input_lv["cap_sgen_lv2"]
    cap_sgen_lv3 = dict_input_lv["cap_sgen_lv3"]
    # load: P, Q
    arr_lv1_p = dict_input_lv["arr_lv1_p"]
    arr_lv1_q = dict_input_lv["arr_lv1_q"]
    arr_lv2_p = dict_input_lv["arr_lv2_p"]
    arr_lv2_q = dict_input_lv["arr_lv2_q"]
    arr_lv3_p = dict_input_lv["arr_lv3_p"]
    arr_lv3_q = dict_input_lv["arr_lv3_q"]
    # sgen: P
    arr_lv1_sgen = dict_input_lv["arr_lv1_sgen"]
    arr_lv2_sgen = dict_input_lv["arr_lv2_sgen"]
    arr_lv3_sgen = dict_input_lv["arr_lv3_sgen"]
    ...
    alpha = m.addVars(n_trafo, lb = -GRB.INFINITY, name = "alpha")
    beta = m.addVars(n_trafo, lb = -GRB.INFINITY, name = "beta") 
    alpha_pos = m.addVars(n_trafo, lb = -GRB.INFINITY, name = "alpha_positive") # flow in LV
    beta_pos = m.addVars(n_trafo, lb = -GRB.INFINITY, name = "beta_positive") # flow in LV
    ...
    #------------------------- Constraints Trafos -----------------------------
    # power flow balance
    m.addConstrs(P_trafo_mv[t] == P_trafo_lv[t] + alpha[t] for t in range(n_trafo))
    m.addConstrs(Q_trafo_mv[t] == Q_trafo_lv[t] + beta[t] for t in range(n_trafo))
    m.addConstrs(alpha_pos[t] >= alpha[t] for t in range(n_trafo))
    m.addConstrs(alpha_pos[t] >= -alpha[t] for t in range(n_trafo))
    m.addConstrs(beta_pos[t] >= beta[t] for t in range(n_trafo))
    m.addConstrs(beta_pos[t] >= -beta[t] for t in range(n_trafo))
    
    # voltage relations: for easier Benders decomposition here
    m.addConstr(v2_trafo[0]*turns_ratio**2-v2_lv1[3] == 2*(r_ohm_160kva*P_trafo_lv[0]+x_ohm_160kva*Q_trafo_lv[0]))
    m.addConstr(v2_trafo[1]*turns_ratio**2-v2_lv2[62] ==2*(r_ohm_630kva*P_trafo_lv[1]+x_ohm_630kva*Q_trafo_lv[1]))
    m.addConstr(v2_trafo[2]*turns_ratio**2-v2_lv3[104]==2*(r_ohm_630kva*P_trafo_lv[2]+x_ohm_630kva*Q_trafo_lv[2]))    
    
    # ----------------------- objective function -----------------------------  
    M = 100
    penalty = M * (quicksum(alpha_pos[t] for t in range(n_trafo)) + quicksum(beta_pos[t] for t in range(n_trafo)))
    obj_quad = quicksum((arr_lv1_sgen[i]-p_sgen_lv1[i])*(arr_lv1_sgen[i]-p_sgen_lv1[i]) for i in range(n_bus_lv1_vec)) + \
          ...
          quicksum((arr_lv3_sgen[i]-p_sgen_lv3[i])*(arr_lv3_sgen[i]-p_sgen_lv3[i]) for i in range(n_bus_lv3_vec)) + penalty
    obj_lin = quicksum((arr_lv1_sgen[i]-p_sgen_lv1[i]) for i in range(n_bus_lv1_vec)) + \
          ...
          quicksum((arr_lv3_sgen[i]-p_sgen_lv3[i]) for i in range(n_bus_lv3_vec)) + penalty
    ...
    obj_prop = quicksum((arr_lv1_sgen[i]-p_sgen_lv1[i])*(arr_lv1_sgen[i]-p_sgen_lv1[i])*(arr_sgen_lv1_inv[i]) for i in range(n_bus_lv1_vec)) + \
          ...
          quicksum((arr_lv3_sgen[i]-p_sgen_lv3[i])*(arr_lv3_sgen[i]-p_sgen_lv3[i])*(arr_sgen_lv3_inv[i]) for i in range(n_bus_lv3_vec)) + penalty
    if objective == "obj_lin":
        m.setObjective(obj_lin, GRB.MINIMIZE)
    elif objective == "obj_quad":
        m.setObjective(obj_quad, GRB.MINIMIZE)
...
def opf_mv_lv(dict_input, objective = "obj_prop", epsilon = 1E-4):
    ...
    cap_sgen_mv_agg = cap_sgen_mv.copy()
    cap_sgen_mv_agg[0] = cap_sgen_lv1.sum()
    cap_sgen_mv_agg[4] = cap_sgen_lv2.sum()
    cap_sgen_mv_agg[8] = cap_sgen_lv3.sum()
    # load: P, Q
    ...
    arr_mv_p_agg = arr_mv_p.copy()
    arr_mv_q_agg = arr_mv_q.copy()
    arr_mv_p_agg[0] = arr_lv1_p.sum()
    arr_mv_q_agg[0] = arr_lv1_q.sum()
    arr_mv_p_agg[4] = arr_lv2_p.sum()
    arr_mv_q_agg[4] = arr_lv2_q.sum()
    arr_mv_p_agg[8] = arr_lv3_p.sum()
    arr_mv_q_agg[8] = arr_lv3_q.sum()
    # sgen: P
    ...
    arr_mv_sgen_agg = arr_mv_sgen.copy()
    arr_mv_sgen_agg[0] = arr_lv1_sgen.sum()
    arr_mv_sgen_agg[4] = arr_lv2_sgen.sum()
    arr_mv_sgen_agg[8] = arr_lv3_sgen.sum()    
    dict_input_mv = {}
    dict_input_mv["cap_sgen_mv"] = cap_sgen_mv_agg
    dict_input_mv["arr_mv_p"] = arr_mv_p_agg
    dict_input_mv["arr_mv_q"] = arr_mv_q_agg
    dict_input_mv["arr_mv_sgen"] = arr_mv_sgen_agg
    dict_output_opf_mv = opf_mv(dict_input_mv, objective = objective)
    # =============== optimize LV ============================================
    P_trafo_mv = dict_output_opf_mv["P_trafo_mv"]
    Q_trafo_mv = dict_output_opf_mv["Q_trafo_mv"]
    v2_trafo = dict_output_opf_mv["v2_trafo"]
    dict_input_lv = {}
    dict_input_lv["cap_sgen_lv1"] = cap_sgen_lv1
    dict_input_lv["cap_sgen_lv2"] = cap_sgen_lv2 
    dict_input_lv["cap_sgen_lv3"] = cap_sgen_lv3
    # load: P, Q
    dict_input_lv["arr_lv1_p"] = arr_lv1_p 
    dict_input_lv["arr_lv1_q"] = arr_lv1_q 
    dict_input_lv["arr_lv2_p"] = arr_lv2_p
    dict_input_lv["arr_lv2_q"] = arr_lv2_q
    dict_input_lv["arr_lv3_p"] = arr_lv3_p
    dict_input_lv["arr_lv3_q"] = arr_lv3_q
    # sgen: P
    dict_input_lv["arr_lv1_sgen"] = arr_lv1_sgen 
    dict_input_lv["arr_lv2_sgen"] = arr_lv2_sgen 
    dict_input_lv["arr_lv3_sgen"] = arr_lv3_sgen 
    dict_output_opf_lv = opf_lv(dict_input_lv, v2_trafo, P_trafo_mv, Q_trafo_mv, objective = objective)
    # ==================== output ============================================    
    dict_output = {}
    dict_output["p_sgen_mv"] = dict_output_opf_mv["p_sgen_mv"]
    dict_output["p_sgen_lv1"] = dict_output_opf_lv["p_sgen_lv1"]
    dict_output["p_sgen_lv2"] = dict_output_opf_lv["p_sgen_lv2"]
    dict_output["p_sgen_lv3"] = dict_output_opf_lv["p_sgen_lv3"]
    dict_output["q_sgen_mv"] = dict_output_opf_mv["q_sgen_mv"]
    dict_output["q_sgen_lv1"] = dict_output_opf_lv["q_sgen_lv1"]
    dict_output["q_sgen_lv2"] = dict_output_opf_lv["q_sgen_lv2"]
    dict_output["q_sgen_lv3"] = dict_output_opf_lv["q_sgen_lv3"]
    dict_output["vm_pu_mv"] = dict_output_opf_mv["vm_pu_mv"]
    dict_output["vm_pu_lv1"] = dict_output_opf_lv["vm_pu_lv1"]
    dict_output["vm_pu_lv2"] = dict_output_opf_lv["vm_pu_lv2"]
    dict_output["vm_pu_lv3"] = dict_output_opf_lv["vm_pu_lv3"]
    dict_output["P_trafo_lv"] = dict_output_opf_lv["P_trafo_lv"]
    dict_output["Q_trafo_lv"] = dict_output_opf_lv["Q_trafo_lv"]
    dict_output["loading_trafo"] = dict_output_opf_lv["loading_trafo"]
    dict_output["loading_line_mv"] = dict_output_opf_mv["loading_line_mv"]
    dict_output["loading_line_lv1"] = dict_output_opf_lv["loading_line_lv1"]
    dict_output["loading_line_lv2"] = dict_output_opf_lv["loading_line_lv2"]
    dict_output["loading_line_lv3"] = dict_output_opf_lv["loading_line_lv3"]
    dict_output["p_curt_mv"] = np.array([arr_mv_sgen[i]-dict_output_opf_mv["p_sgen_mv"][i] for i in range(n_bus_mv)])
    dict_output["p_curt_lv1"] = np.array([arr_lv1_sgen[i]-dict_output_opf_lv["p_sgen_lv1"][i] for i in range(n_bus_lv1_vec)])
    dict_output["p_curt_lv2"] = np.array([arr_lv2_sgen[i]-dict_output_opf_lv["p_sgen_lv2"][i] for i in range(n_bus_lv2_vec)])
    dict_output["p_curt_lv3"] = np.array([arr_lv3_sgen[i]-dict_output_opf_lv["p_sgen_lv3"][i] for i in range(n_bus_lv3_vec)])
    arr_sgen_mv_inv = 1/arr_mv_sgen
    arr_sgen_mv_inv[arr_sgen_mv_inv == inf] = 0
    arr_sgen_lv1_inv = 1/arr_lv1_sgen
    arr_sgen_lv1_inv[arr_sgen_lv1_inv == inf] = 0
