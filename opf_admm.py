...
def opf_mv(dict_input_mv, lambda1, lambda2, lambda3, var_bd_lv1, var_bd_lv2, var_bd_lv3, rho, objective): 
    '''
    ...
    modification = lambda1[0]*P_trafo_mv[0] + lambda1[1]*Q_trafo_mv[0] + lambda1[2]*v2_mv[1]\
        + lambda2[0]*P_trafo_mv[1] + lambda2[1]*Q_trafo_mv[1] + lambda2[2]*v2_mv[5]\
            + lambda3[0]*P_trafo_mv[2] + lambda3[1]*Q_trafo_mv[2] + lambda3[2]*v2_mv[9]\
                + 0.5 * (
                     rho[0] * (P_trafo_mv[0] - var_bd_lv1[0]) * (P_trafo_mv[0] - var_bd_lv1[0]) 
                    + rho[1] * (Q_trafo_mv[0] - var_bd_lv1[1]) * (Q_trafo_mv[0] - var_bd_lv1[1])
                    + rho[2] * (v2_mv[1] - var_bd_lv1[2]) * (v2_mv[1] - var_bd_lv1[2])
                    + rho[3] * (P_trafo_mv[1] - var_bd_lv2[0]) * (P_trafo_mv[1] - var_bd_lv2[0]) 
                    + rho[4] * (Q_trafo_mv[1] - var_bd_lv2[1]) * (Q_trafo_mv[1] - var_bd_lv2[1])
                    + rho[5] * (v2_mv[5] - var_bd_lv2[2]) * (v2_mv[5] - var_bd_lv2[2])
                    + rho[6] * (P_trafo_mv[2] - var_bd_lv3[0]) * (P_trafo_mv[2] - var_bd_lv3[0]) 
                    + rho[7] * (Q_trafo_mv[2] - var_bd_lv3[1]) * (Q_trafo_mv[2] - var_bd_lv3[1])
                    + rho[8] * (v2_mv[9] - var_bd_lv3[2]) * (v2_mv[9] - var_bd_lv3[2])
                        )
...
def opf_lv1(dict_input_lv1, lambda1, var_bd_mv, rho, objective):
    '''
    ...
    # --------------- dd obj modification ------------------------------------
    modification = -(lambda1[0]*P_trafo_lv + lambda1[1]*Q_trafo_lv + lambda1[2]*v2_trafo) + 0.5*(
        rho[0] * (P_trafo_lv-var_bd_mv[0])*(P_trafo_lv-var_bd_mv[0]) + rho[1] * (Q_trafo_lv-var_bd_mv[1])*(Q_trafo_lv-var_bd_mv[1]) + 
        rho[2] * (v2_trafo-var_bd_mv[2])*(v2_trafo-var_bd_mv[2]))       
    # ----------------------- objective function -----------------------------  
...
def opf_lv2(dict_input_lv2, lambda2, var_bd_mv, rho, objective):
    '''
    They can be solved together because of no mutual impact
    '''
    #------------------------  Load Profiles ----------------------------------
    ...
    # --------------- dd obj modification ------------------------------------
    modification = -(lambda2[0]*P_trafo_lv + lambda2[1]*Q_trafo_lv + lambda2[2]*v2_trafo) + 0.5*(
       rho[0] * (P_trafo_lv-var_bd_mv[0])*(P_trafo_lv-var_bd_mv[0]) + rho[1] * (Q_trafo_lv-var_bd_mv[1])*(Q_trafo_lv-var_bd_mv[1]) + 
       rho[2] * (v2_trafo-var_bd_mv[2])*(v2_trafo-var_bd_mv[2]))       
    # ----------------------- objective function -----------------------------  
...
def opf_lv3(dict_input_lv3, lambda3, var_bd_mv, rho, objective):
    '''
    They can be solved together because of no mutual impact
    '''
    #------------------------  Load Profiles ----------------------------------
    ...
    # --------------- dd obj modification ------------------------------------
    modification = -(lambda3[0]*P_trafo_lv + lambda3[1]*Q_trafo_lv + lambda3[2]*v2_trafo) + 0.5*(
        rho[0] * (P_trafo_lv-var_bd_mv[0])*(P_trafo_lv-var_bd_mv[0]) + rho[1] * (Q_trafo_lv-var_bd_mv[1])*(Q_trafo_lv-var_bd_mv[1]) + 
        rho[2] * (v2_trafo-var_bd_mv[2])*(v2_trafo-var_bd_mv[2])) 
    # ----------------------- objective function -----------------------------  
...
def opf_mv_lv(dict_input, objective, epsilon = 1E-4):
    ...
    while (resid >= epsilon and n_iterations < max_iterations):
        ...
        dict_output_opf_mv = opf_mv(dict_input_mv, lambda1, lambda2, lambda3, var_bd_lv1, var_bd_lv2, var_bd_lv3, rho, objective)
        ...
        dict_output_opf_lv1 = opf_lv1(dict_input_lv1, lambda1, np.array([P_trafo_mv[0],Q_trafo_mv[0],v2_trafo_mv[0]]), rho[0:3], objective)
        dict_output_opf_lv2 = opf_lv2(dict_input_lv2, lambda2, np.array([P_trafo_mv[1],Q_trafo_mv[1],v2_trafo_mv[1]]), rho[3:6], objective)
        dict_output_opf_lv3 = opf_lv3(dict_input_lv3, lambda3, np.array([P_trafo_mv[2],Q_trafo_mv[2],v2_trafo_mv[2]]), rho[6:9], objective)
        # primal residual
        ...
        # dual residual
        resid_dual = rho * abs(np.array([P_trafo_lv1, Q_trafo_lv1, v2_trafo_lv1,P_trafo_lv2, Q_trafo_lv2, v2_trafo_lv2,P_trafo_lv3, Q_trafo_lv3, v2_trafo_lv3])
        ...
        lambda1 += rho[0:3] * np.array([P_trafo_mv[0]-P_trafo_lv1, Q_trafo_mv[0]-Q_trafo_lv1, v2_trafo_mv[0]-v2_trafo_lv1])
        lambda2 += rho[3:6] * np.array([P_trafo_mv[1]-P_trafo_lv2, Q_trafo_mv[1]-Q_trafo_lv2, v2_trafo_mv[1]-v2_trafo_lv2])
        lambda3 += rho[6:9] * np.array([P_trafo_mv[2]-P_trafo_lv3, Q_trafo_mv[2]-Q_trafo_lv3, v2_trafo_mv[2]-v2_trafo_lv3])
        # update rho
        for i in range(9):
